...
export const Session = {
  ...
  get(): User | null {
    try {
      ...
      if (!raw) return null;
      const p = JSON.parse(raw) as { user: User; rememberMe: boolean; setAt: number };
  ...
  isLoggedIn():   boolean { return this.get() !== null; },
...
export const AuthDB = {
  ...
  async register(data: {
    ...
    if (error) {
      if (error.code === "23505") return { ok: false, msg: "An account with this email already exists." };
  ...
  async loginWithOAuth(data: {
    ...
    let { data: existing } = await supabase
      .from("users").select("*").eq("firebase_uid", data.oauthId).maybeSingle();
    ...
    if (existing) {
      const { data: updated, error } = await supabase
        .from("users")
        .update({ firebase_uid: data.oauthId, oauth_provider: data.provider, oauth_id: data.oauthId, email_verified: true })
        .eq("id", existing.id).select().single();
      if (error) return { ok: false, msg: error.message, isNew: false };
      return { ok: true, user: toUser(updated), isNew: false };
    }

    if (!data.role) return { ok: false, msg: "Please select Host or Guest before signing in.", isNew: true };

    const { data: created, error } = await supabase
      .from("users")
      .insert({
        firebase_uid: data.oauthId, email, role: data.role,
        first_name: data.firstName, last_name: data.lastName,
        company: "", country: "", phone: "",
        avatar: data.avatar ?? (data.firstName[0] + (data.lastName?.[0] ?? "")).toUpperCase(),
        oauth_provider: data.provider, oauth_id: data.oauthId,
        email_verified: true, marketing_opt_in: false,
        wishlist: [], bookings: [],
      })
      .select().single();
  ...
  async update(id: string, data: Partial<Omit<User, "id" | "createdAt">>): Promise<User | null> {
    ...
    if (data.marketingOptIn !== undefined) p.marketing_opt_in  = data.marketingOptIn;
    ...
    if (error) { console.error("AuthDB.update:", error.message); return null; }
...
export const ListingsDB = {
  ...
  async add(data: Omit<Listing, "id" | "rating" | "reviewCount" | "featured" | "available" | "createdAt">): Promise<Listing> {
    const { data: row, error } = await supabase
      .from("listings")
      .insert({
        host_id: data.hostId, host_name: data.hostName,
        name: data.name, description: data.description,
        location: data.location, city: data.city, country: data.country,
        category: data.category,
        price_per_night: Number(data.pricePerNight),
        bedrooms: Number(data.bedrooms), bathrooms: Number(data.bathrooms),
        max_guests: Number(data.maxGuests),
        amenities: data.amenities, tags: data.tags, images: data.images,
        rating: 0, review_count: 0, featured: false, available: true,
      })
      .select().single();
  ...
  async update(id: string, data: Partial<Listing>): Promise<Listing | null> {
    ...
    const { data: updated, error } = await supabase
      .from("listings").update(p).eq("id", id).select().single();
    if (error) { console.error("ListingsDB.update:", error.message); return null; }
  ...
  async byHost(hostId: string): Promise<Listing[]> {
    const { data, error } = await supabase
      .from("listings").select("*").eq("host_id", hostId)
      .order("created_at", { ascending: false });
    if (error) { console.error("ListingsDB.byHost:", error.message); return []; }
    return (data ?? []).map(toListing);
  },

  async all(): Promise<Listing[]> {
    const { data, error } = await supabase
      .from("listings").select("*").eq("available", true)
      .order("featured", { ascending: false })
      .order("created_at", { ascending: false });
    if (error) { console.error("ListingsDB.all:", error.message); return []; }
    return (data ?? []).map(toListing);
  },
  ...
  async search(query: string, guests?: number, category?: string): Promise<Listing[]> {
    ...
    if (error) { console.error("ListingsDB.search:", error.message); return []; }
    return (data ?? []).map(toListing);
  },
};
...
export const BookingsDB = {
  ...
  async add(data: {
    ...
    if (guest) await AuthDB.update(data.guestId, { bookings: [...guest.bookings, row.id] });
  ...
  async byGuest(guestId: string): Promise<Booking[]> {
    const { data, error } = await supabase
      .from("bookings").select("*").eq("guest_id", guestId)
      .order("created_at", { ascending: false });
    if (error) { console.error("BookingsDB.byGuest:", error.message); return []; }
    return (data ?? []).map(toBooking);
  },

  async byHost(hostId: string): Promise<Booking[]> {
    const { data, error } = await supabase
      .from("bookings").select("*").eq("host_id", hostId)
      .order("created_at", { ascending: false });
    if (error) { console.error("BookingsDB.byHost:", error.message); return []; }
    return (data ?? []).map(toBooking);
  },
  ...
  async all(): Promise<Booking[]> {
    ...
    if (error) { console.error("BookingsDB.all:", error.message); return []; }
    return (data ?? []).map(toBooking);
  },
